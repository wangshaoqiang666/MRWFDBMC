...
#             layers.append(nn.Linear(dims[i], dims[i + 1], dtype=torch.double))
#             if i < len(dims) - 2:
#                 layers.append(nn.BatchNorm1d(dims[i + 1], dtype=torch.double))
#                 layers.append(nn.ELU())
#                 layers.append(nn.Dropout(dropout))
#         self.mlp = nn.Sequential(*layers)
#
#     def forward(self, x):
#         # 输入x形状: [batch_size, input_dim] 或 [num_nodes, input_dim]
#         return self.mlp(x)
